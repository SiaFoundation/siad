...
func main() {
	...
	hostAddr := flag.String("hostAddr", ":0", "address for the host to listen on")
	...
	if *checkpoint != "" {
		...
		if err := os.RemoveAll(filepath.Join(*dir, "chain")); err != nil {
			log.Fatal(err)
		} else if err := os.RemoveAll(filepath.Join(*dir, "wallet")); err != nil {
			log.Fatal(err)
		}
	}

	n, err := newNode(*gatewayAddr, *hostAddr, *dir, initCheckpoint)
	if err != nil {
		log.Fatal(err)
	}
	defer func() {
	...
	log.Println("host: listening on", n.h.Addr())
	log.Printf("host: public key %v", n.h.PublicKey())
	go func() {
