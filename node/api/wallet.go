...
func encryptionKeys(seedStr string) (validKeys []crypto.SiaKey) {
	...
	validKeys = append(validKeys, crypto.NewWalletKey(crypto.TypeDefaultWallet, crypto.HashObject(seedStr)))
	return validKeys
}
...
func (api *API) walletInitHandler(w http.ResponseWriter, req *http.Request, _ httprouter.Params) {
	var encryptionKey crypto.SiaKey
	if req.FormValue("encryptionpassword") != "" {
		encryptionKey = crypto.NewWalletKey(crypto.TypeDefaultWallet, crypto.HashObject(req.FormValue("encryptionpassword")))
	}
