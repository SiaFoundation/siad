...
func TestRenterContractRecovery(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Dir(newPath), persist.DefaultDiskPermissions); err != nil {
		t.Fatal(err)
	}
	if err := ioutil.WriteFile(newPath, siaFile, persist.DefaultDiskPermissions); err != nil {
		t.Fatal(err)
	}
