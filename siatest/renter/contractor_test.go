...
func TestRenterContractRecovery(t *testing.T) {
	...
	oldPath := filepath.Join(r.Dir, modules.RenterDir, modules.HomeFolderRoot, modules.SiaFilesRoot, lf.FileName()+modules.SiaFileExtension)
	siaFile, err := ioutil.ReadFile(oldPath)
	if err != nil {
		t.Fatal(err)
	}
	newRenterDir := filepath.Join(testDir, "renter")
	newPath := filepath.Join(newRenterDir, modules.RenterDir, modules.HomeFolderRoot, modules.SiaFilesRoot, lf.FileName()+modules.SiaFileExtension)
