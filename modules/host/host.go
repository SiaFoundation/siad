...
func (h *Host) RegistryGet(sid modules.EntryID) (types.SiaPublicKey, modules.SignedRegistryValue, bool) {
	err := h.tg.Add()
	if err != nil {
...
func (h *Host) RegistryUpdate(rv modules.SignedRegistryValue, pubKey types.SiaPublicKey, expiry types.BlockHeight) (modules.SignedRegistryValue, error) {
	...
	if h.dependencies.Disrupt("RegistryUpdateLyingHost") {
		_, srv, found := h.staticRegistry.Get(modules.RegistryEntryID(pubKey, rv.Tweak))
		if found {
