...
func blankMockHostTester(d modules.Dependencies, name string) (*hostTester, error) {
	...
	h, err := NewCustomHost(d, cs, g, tp, w, sm, "localhost:0", filepath.Join(testdir, modules.HostDir))
	if err != nil {
		return nil, err
	}
	/*
...
func TestNilValues(t *testing.T) {
	...
	_, err = New(nil, ht.gateway, ht.tpool, ht.wallet, ht.sm, "localhost:0", hostDir)
	if err != errNilCS {
		t.Fatal("could not trigger errNilCS")
	}
	_, err = New(ht.cs, nil, ht.tpool, ht.wallet, ht.sm, "localhost:0", hostDir)
	if err != errNilGateway {
		t.Fatal("Could not trigger errNilGateay")
	}
	_, err = New(ht.cs, ht.gateway, nil, ht.wallet, ht.sm, "localhost:0", hostDir)
	if err != errNilTpool {
		t.Fatal("could not trigger errNilTpool")
	}
	_, err = New(ht.cs, ht.gateway, ht.tpool, nil, ht.sm, "localhost:0", hostDir)
	if err != errNilWallet {
...
func TestSetAndGetInternalSettings(t *testing.T) {
	...
	rebootHost, err := New(ht.cs, ht.gateway, ht.tpool, ht.wallet, ht.sm, "localhost:0", filepath.Join(ht.persistDir, modules.HostDir))
	if err != nil {
		t.Fatal(err)
	}
...
func TestSetAndGetSettings(t *testing.T) {
	...
	rebootHost, err := New(ht.cs, ht.tpool, ht.wallet, ht.sm, "localhost:0", filepath.Join(ht.persistDir, modules.HostDir))
	if err != nil {
		t.Fatal(err)
	}
...
func TestPersistentSettings(t *testing.T) {
	...
	h, err := New(ht.cs, ht.tpool, ht.wallet, ht.sm, "localhost:0", filepath.Join(ht.persistDir, modules.HostDir))
	if err != nil {
		t.Fatal(err)
	}
