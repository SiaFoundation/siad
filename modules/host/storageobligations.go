...
func (h *Host) managedAddStorageObligation(so storageObligation, renewal bool) error {
	...
	err := func() error {
		...
		h.financialMetrics.PotentialAccountFunding = h.financialMetrics.AccountFunding.Add(so.AccountFunding)
...
func (h *Host) managedModifyStorageObligation(so storageObligation, sectorsRemoved []crypto.Hash, sectorsGained map[crypto.Hash][]byte) error {
	...
	h.financialMetrics.PotentialAccountFunding = h.financialMetrics.AccountFunding.Sub(oldSO.AccountFunding)
