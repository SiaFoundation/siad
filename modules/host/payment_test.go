...
func TestVerifyEAFundRevision(t *testing.T) {
	...
	err = verifyEAPaymentRevision(curr, payment, height, amount)
	if err != nil {
	...
	err = verifyEAPaymentRevision(curr, badPayment, height, amount)
	...
	err = verifyEAPaymentRevision(badCurr, payment, height, amount)
	...
	err = verifyEAPaymentRevision(curr, payment, height, amount.Add64(1))
	if !errors.Contains(err, ErrHighRenterValidOutput) {
		t.Fatalf("Expected '%v' but received '%v'", string(ErrHighRenterValidOutput), err)
	}
