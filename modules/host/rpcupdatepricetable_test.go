...
func TestPruneExpiredPriceTables(t *testing.T) {
	...
	if !exists {
		t.Fatal("Price table doesn't exist in host's map of guaranteed tables")
	}
...
func TestUpdatePriceTableRPC(t *testing.T) {
	...
	go func() {
		...
		pt, err := negotiatePriceTable(ht.host)
		if err != nil {
			t.Error("failed to get price table")
			return
		}
		...
		if ptc.Equals(types.ZeroCurrency) {
			t.Log(ptc)
			t.Error("Expected the cost of the updatePriceTableRPC to be set")
			return
		}
...
func negotiatePriceTable(host *Host) (*modules.RPCPriceTable, error) {
	// create a test stream
	stream, err := createTestStream(host)
	if err != nil {
		return nil, err
	}
	defer stream.Close()
