...
const (
	// bucketSize defines the range of blocks a bucket spans, buckets rotate
	// every 'bucketSize' blocks
	bucketSize = 20
...
func (mb *memoryBucket) save(fp crypto.Hash, expiry types.BlockHeight) {
	if expiry <= mb.bucketHeight {
		(*mb.current)[fp] = struct{}{}
	} else {
		(*mb.next)[fp] = struct{}{}
	}
}
...
// buckets
func (mb *memoryBucket) has(fp crypto.Hash) bool {
	_, exists := mb.bucketOne[fp]
	if !exists {
		_, exists = mb.bucketTwo[fp]
	}
	return exists
}
...
// save will add the given fingerprint to the appropriate bucket
func (fb *fileBucket) save(fp *crypto.Hash, expiry types.BlockHeight) error {
	fpb := make([]byte, fingerprintSize)
	copy(fpb, encoding.Marshal(*fp))

	var err error
	if expiry <= fb.bucketHeight {
...
func (fb *fileBucket) all() ([]*crypto.Hash, error) {
