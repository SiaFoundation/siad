...
type Renter interface {
	...
	DeleteFile(siaPath types.SiaPath) error
	...
	File(siaPath types.SiaPath) (FileInfo, error)
	...
	RenameFile(siaPath, newSiaPath types.SiaPath) error
	...
	SetFileTrackingPath(siaPath types.SiaPath, newPath string) error
	...
	// resource.
	Streamer(siapath types.SiaPath) (string, Streamer, error)

	// Upload uploads a file using the input parameters.
	Upload(FileUploadParams) error

	// CreateDir creates a directory for the renter
	CreateDir(siaPath types.SiaPath) error

	// DeleteDir deletes a directory from the renter
	DeleteDir(siaPath types.SiaPath) error

	// DirList lists the directories and the files stored in a siadir
	DirList(siaPath types.SiaPath) ([]DirectoryInfo, []FileInfo, error)
}
