...
func (s *streamer) managedFillCache() bool {
	...
	d, err := s.r.managedNewDownload(downloadParams{
		...
		disableDiskFetch:  s.staticDisableDiskFetch,
		file:              s.staticFile,
...
// the sia network.
func (r *Renter) Streamer(siaPath modules.SiaPath, disableDiskFetch bool) (string, modules.Streamer, error) {
	...
	s := r.managedStreamer(snap, disableDiskFetch)
...
// its cache.
func (r *Renter) managedStreamer(snapshot *siafile.Snapshot, disableDiskFetch bool) modules.Streamer {
	s := &streamer{
		staticFile: snapshot,
		r:          r,
		...
		staticDisableDiskFetch: disableDiskFetch,
