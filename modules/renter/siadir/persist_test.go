...
func TestCreateReadMetadataUpdate(t *testing.T) {
	...
	// Check path
	path2 := sd.siaPath.SiaDirMetadataSysPath(sd.metadata.RootDir)
	if path != path2 {
...
func TestCreateReadDeleteUpdate(t *testing.T) {
	...
	// Compare values
	siaDirPath := sd.siaPath.SiaDirSysPath(sd.metadata.RootDir)
	if path != siaDirPath {
...
func testApply(t *testing.T, siadir *SiaDir, apply func(...writeaheadlog.Update) error) {
	...
	metadata.Health = 1.0
	path := siadir.siaPath.SiaDirMetadataSysPath(siadir.metadata.RootDir)
	...
	// Open file.
	sd, err := LoadSiaDir(metadata.RootDir, siadir.siaPath, modules.ProdDependencies, siadir.wal)
	if err != nil {
