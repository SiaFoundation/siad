...
// file directory is updated periodically. Along with the health, the metadata
// for the files and directories is also updated. The metadata information for a
// directory is stored in the .siadir metadata file and contains directory
// specific and aggregate information. The aggregate fields are the worst values
// for any of the files and sub directories. This is true for all directories
// which, for example, means the health of top level directory of the renter is
// the health of the worst file in the renter. For health and stuck health the
// worst value is the highest value, for timestamp values the oldest timestamp
// is the worst value, and for aggregate values (ie NumStuckChunks) it will be
// the sum of all the files and sub directories. The health loop keeps the
// renter file directory updated by following the path of oldest
// LastHealthCheckTime and then calling `managedBubbleMetadata` or
// `threadedBubbleMetadata`, to be referred to as bubble, on that directory.
// When a directory is bubbled, the metadata information is recalculated and
// saved to disk and then bubble is called on the parent directory until the top
// level directory is reached. If during a bubble a file is found that meets the
...
// includes uploads. The repair loop uses a `directoryHeap` which is a max heap
// of directory elements sorted by health. The repair loop follows the following
// sudo code:
//
// 1. Add backup chunks to heap
//
// 2. Check if file system is healthy and no chunks in upload heap
//     Sleep until triggered by upload or repair needed
//
// 3. Add chunks to heap
//
// 4. Repair chunks
//
// 5. Call bubble to update file system
//
...
// currently in the upload heap, then the repair loop sleeps until it is
// triggered by a new upload or a repair is needed. Chunks are added to the
// upload heap by popping the directory off the directory heap and adding any
// chunks that are a worse health than the next directory in the directory heap.
// This continues until the `MaxUploadHeapChunks` is met. The repair loop will
// then repair those chunks and call bubble on the directories that chunks were
// added from to keep the file system updated. This will continue until the file
// system is healthy, which means all files have a health less than the
// `RepairThreshold`.
...
// and then will randomly add one stuck chunk to the heap. Stuck chunks are
// priority in the heap, so limiting it to `MaxStuckChunksInHeap` at a time
// prevents the heap from being saturated with stuck chunks that potentially
// cannot be repaired which would cause no other files to be repaired. If the
// repair of a stuck chunk is successful, a signal is sent to the stuck loop and
// another stuck chunk is added to the heap. If the repair wasn't successful,
// the stuck loop will wait for the `repairStuckChunkInterval` to pass and then
// try another random stuck chunk. If the stuck loop doesn't find any stuck
// chunks, it will sleep until a bubble triggers it by finding a stuck chunk.
