...
func (w *worker) managedDownload(udc *unfinishedDownloadChunk) {
	...
	d, err := w.renter.hostContractor.Downloader(w.hostPubKey, w.renter.tg.StopChan())
	if err != nil {
	...
	root := udc.staticChunkMap[w.hostPubKey.String()].root
	...
	pieceIndex := udc.staticChunkMap[w.hostPubKey.String()].index
...
func (udc *unfinishedDownloadChunk) managedUnregisterWorker(w *worker) {
	udc.mu.Lock()
	udc.piecesRegistered--
	udc.pieceUsage[udc.staticChunkMap[w.hostPubKey.String()].index] = false
	udc.mu.Unlock()
}
...
func (w *worker) ownedProcessDownloadChunk(udc *unfinishedDownloadChunk) *unfinishedDownloadChunk {
	...
	pieceData, workerHasPiece := udc.staticChunkMap[w.hostPubKey.String()]
