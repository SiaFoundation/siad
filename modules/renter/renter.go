...
func renterBlockingStartup(g modules.Gateway, cs modules.ConsensusSet, tpool modules.TransactionPool, hdb hostDB, w modules.Wallet, hc hostContractor, sm *siamux.SiaMux, persistDir string, deps modules.Dependencies) (*Renter, error) {
	if g == nil {
...
func NewCustomRenter(g modules.Gateway, cs modules.ConsensusSet, tpool modules.TransactionPool, hdb hostDB, w modules.Wallet, hc hostContractor, sm *siamux.SiaMux, persistDir string, deps modules.Dependencies) (*Renter, <-chan error) {
	errChan := make(chan error, 1)

	// Blocking startup.
	r, err := renterBlockingStartup(g, cs, tpool, hdb, w, hc, sm, persistDir, deps)
	if err != nil {
		errChan <- err
		return nil, errChan
	}
...
func New(g modules.Gateway, cs modules.ConsensusSet, wallet modules.Wallet, tpool modules.TransactionPool, sm *siamux.SiaMux, persistDir string) (*Renter, <-chan error) {
	errChan := make(chan error, 1)
	...
	renter, errChanRenter := NewCustomRenter(g, cs, tpool, hdb, wallet, hc, sm, persistDir, modules.ProdDependencies)
