...
type (
	...
	EncryptionManager interface {
		...
		Encrypt(masterKey crypto.SiaKey) (Seed, error)
		...
		InitFromSeed(masterKey crypto.SiaKey, seed Seed) error
	...
	KeyManager interface {
		...
		Load033xWallet(crypto.SiaKey, string) error
		...
		LoadSeed(crypto.SiaKey, Seed) error
		...
		// become spendable.
		LoadSiagKeys(crypto.SiaKey, []string) error
