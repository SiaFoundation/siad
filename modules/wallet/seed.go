...
func createSeedFile(masterKey crypto.SiaKey, seed modules.Seed) seedFile {
	var sf seedFile
...
func decryptSeedFile(masterKey crypto.SiaKey, sf seedFile) (seed modules.Seed, err error) {
...
// the wallet.
func (w *Wallet) LoadSeed(masterKey crypto.SiaKey, seed modules.Seed) error {
	if err := w.tg.Add(); err != nil {
		return err
	}
	defer w.tg.Done()
